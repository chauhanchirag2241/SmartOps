...
public sealed class ClassFeeAmountController : ControllerBase
{
    ...
    public async Task<IActionResult> GetClassSummaries([FromQuery] Guid academicYearId, CancellationToken ct)
    {
        var result = await _service.GetClassSummariesAsync(academicYearId, ct).ConfigureAwait(false);
    ...
    public async Task<IActionResult> GetClassAmounts(Guid classId, [FromQuery] Guid academicYearId, CancellationToken ct)
    {
        var result = await _service.GetClassAmountsAsync(classId, academicYearId, ct).ConfigureAwait(false);
        return result.IsSuccess ? Ok(result.Value) : BadRequest(result.Error);
    }
