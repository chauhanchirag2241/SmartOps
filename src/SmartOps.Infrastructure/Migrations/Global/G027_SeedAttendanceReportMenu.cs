...
[Tags("Global")]
[Migration(27, "Global — seed attendance report menu")]
public sealed class G027_SeedAttendanceReportMenu : Migration
{
...
WHERE lower(trim(r.name)) = lower(trim('{RoleNames.SmartOpsAdmin}')) AND m.code = '{MenuCodes.AttendanceReport}'
  AND NOT EXISTS (
