...
[Tags("Global")]
[Migration(32, "Global — seed teacher timetable report under Reports")]
public sealed class G032_SeedTeacherTimetableReportMenu : Migration
{
...
""");
        ...
        {
            InsertPerm(roleCode, view: true, export: true);
        }
...
WHERE r.code = 'ADMIN' AND m.code = '{MenuCodes.TeacherTimetableReport}'
  AND NOT EXISTS (
...
""");
    ...
    private void InsertPerm(string roleCode, bool view, bool export)
    {
        DateTimeOffset now = DateTimeOffset.UtcNow;
        Execute.Sql($"""
INSERT INTO {DatabaseConfig.Schema_Global}.{DatabaseConfig.TableRoleMenuPermissions}
    (id, roleid, menuid, canview, canadd, canedit, candelete, canexport, isactive, versionno, createdby, createdon, updatedby, updatedon)
SELECT gen_random_uuid(), r.id, m.id, {(view ? "true" : "false")}, false, false, false, {(export ? "true" : "false")}, true, 1, '{SeedActor}', '{now:O}', '{SeedActor}', '{now:O}'
FROM {DatabaseConfig.Schema_Global}.{DatabaseConfig.TableRoles} r
CROSS JOIN {DatabaseConfig.Schema_Global}.{DatabaseConfig.TableMenus} m
WHERE r.code = '{roleCode}' AND m.code = '{MenuCodes.TeacherTimetableReport}'
  AND NOT EXISTS (
    SELECT 1 FROM {DatabaseConfig.Schema_Global}.{DatabaseConfig.TableRoleMenuPermissions} rp
    WHERE rp.roleid = r.id AND rp.menuid = m.id
  );
